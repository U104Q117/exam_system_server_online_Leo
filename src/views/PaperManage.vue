  ...
  <div class="paper-manage-container">
    <div class="header">
      <div class="header-right">
        ...
        >
          批量刪除 ({{ selectedPapers.length }})
        </el-button>
        <el-button type="primary" @click="goToCreatePage" :icon="Plus" class="create-btn">創建新試卷</el-button>
      </div>
    </div>
    ...
    <div class="paper-list">
      <el-table :data="paperList" v-loading="loading" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="試卷名稱" min-width="200" />
        ...
        <el-table-column prop="description" label="試卷描述" min-width="250" show-overflow-tooltip />
        <el-table-column prop="createTime" label="創建時間" width="180" />
        <el-table-column prop="questionCount" label="題目數量" width="100">
          <template #default="{ row }">
            {{ row.questionCount }} 道
          </template>
        </el-table-column>
        <el-table-column prop="totalScore" label="總分" width="120">
           <template #default="{ row }">
            {{ row.totalScore }} 分
          </template>
        </el-table-column>
        <el-table-column prop="duration" label="考試時長" width="120">
          <template #default="{ row }">
            {{ row.duration }} 分
          </template>
        </el-table-column>
        <el-table-column label="操作" width="250" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.status === 'DRAFT' || row.status === '待發布'" size="small" type="success" @click="updateStatus(row, 'PUBLISHED')" :icon="CaretRight">發布</el-button>
            <el-button v-if="row.status === 'PUBLISHED'" size="small" type="warning" @click="updateStatus(row, 'DRAFT')" :icon="VideoPause">停止</el-button>
            <el-button size="small" type="primary" @click="editPaper(row)" :icon="Edit">編輯</el-button>
            <el-button size="small" type="danger" @click="deletePaper(row)" :icon="Delete">刪除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
