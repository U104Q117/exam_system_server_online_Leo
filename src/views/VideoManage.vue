<template>
  <div class="video-manage-container">
    <div class="search-bar">
      <el-row :gutter="20">
        <el-col :span="6">
          <el-input v-model="searchForm.keyword" placeholder="搜索影片或描述" clearable @input="handleSearch">
            <template #prefix><el-icon><Search /></el-icon></template>
          </el-input>
        </el-col>
        <el-col :span="4">
          <el-select v-model="searchForm.status" placeholder="選擇狀態" clearable @change="handleSearch">
        ...
        <el-col :span="4">
          <el-select v-model="searchForm.uploaderType" placeholder="上傳者類型" clearable @change="handleSearch">
            <el-option label="用戶投稿" :value="1" />
            <el-option label="管理員上傳" :value="2" />
          </el-select>
        </el-col>
        <el-col :span="4">
          <el-select v-model="searchForm.categoryId" placeholder="選擇分類" clearable @change="handleSearch">
        ...
        <el-col :span="6">
          ...
          <el-button type="success" @click="showUploadDialog" icon="Plus">上傳影片</el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <div class="video-list">
      <el-table :data="videoList" v-loading="loading" stripe>
        ...
        <el-table-column label="上傳類型" width="100">
          <template #default="scope">
        ...
        <el-table-column prop="likeCount" label="點讚數" width="80" />
        <el-table-column prop="createdAt" label="建立時間" width="180" />
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="scope">
            <el-button size="small" @click="previewVideo(scope.row)" icon="VideoPlay">預覽</el-button>
            <el-button size="small" @click="editVideo(scope.row)" icon="Edit">編輯</el-button>
            <el-button v-if="scope.row.status === 0" size="small" type="success" @click="auditVideoAction(scope.row, 1)" icon="Check">通過</el-button>
            <el-button v-if="scope.row.status === 0" size="small" type="warning" @click="showRejectDialog(scope.row)" icon="Close">拒絕</el-button>
    ...
    <el-dialog v-model="uploadDialogVisible" title="上傳影片" width="800px" :close-on-click-modal="false">
      <el-form :model="uploadForm" :rules="uploadRules" ref="uploadFormRef" label-width="100px">
        <el-form-item label="影片標題" prop="title"><el-input v-model="uploadForm.title" placeholder="请输入影片標題" /></el-form-item>
        <el-form-item label="影片描述" prop="description"><el-input v-model="uploadForm.description" type="textarea" :rows="3" placeholder="请输入影片描述" /></el-form-item>
        <el-form-item label="影片分類" prop="categoryId">
          <el-cascader v-model="categoryPath" :options="categoryTree" :props="cascaderProps" placeholder="請先選擇一级分類，再選擇二级分類" style="width: 100%" @change="handleCategoryChange" clearable />
        </el-form-item>
        <el-form-item label="影片標籤" prop="tags">
          <div class="tags-input-section">
            <el-input v-model="currentTag" placeholder="輸入標籤後回車添加（最多5個）" @keyup.enter="addTag" @blur="addTag" :disabled="tagList.length >= 5" />
        ...
        <el-form-item label="上傳者名稱" prop="uploaderName"><el-input v-model="uploadForm.uploaderName" placeholder="請輸入上傳者名稱" /></el-form-item>
        <el-form-item label="影片时長" prop="duration">
          <div class="duration-input-wrapper">
            <el-input-number v-model="uploadForm.duration" :min="1" placeholder="例如：300" style="width: 200px;" />
            <span class="duration-unit">秒</span>
            <div class="duration-tips"><el-text type="info" size="small">請輸入影片时長（秒），例如：5分鐘 = 300秒，1小時 = 3600秒</el-text></div>
          </div>
        </el-form-item>
        <el-form-item label="影片文件" prop="videoFile">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleUpload" :loading="uploading">確定上傳</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="previewDialogVisible" :title="currentPreviewVideo.title" width="900px">
      <div class="video-preview" v-if="currentPreviewVideo.fileUrl">
        ...
        <div class="video-info">
          <p><strong>影片描述：</strong>{{ currentPreviewVideo.description }}</p>
          <p><strong>影片分類：</strong>{{ currentPreviewVideo.categoryName }}</p>
          <p><strong>影片標籤：</strong>{{ currentPreviewVideo.tags }}</p>
          <p><strong>上傳者名稱：</strong>{{ currentPreviewVideo.uploaderName }}</p>
          <p><strong>觀看次數：</strong>{{ currentPreviewVideo.viewCount }}</p>
          <p><strong>點讚數：</strong>{{ currentPreviewVideo.likeCount }}</p>
        </div>
      </div>
    </el-dialog>

    <el-dialog v-model="rejectDialogVisible" title="拒絕核審" width="500px">
      <el-form :model="rejectForm" :rules="rejectRules" ref="rejectFormRef" label-width="100px">
        <el-form-item label="拒絕原因" prop="reason"><el-input v-model="rejectForm.reason" type="textarea" :rows="4" placeholder="請輸入拒絕原因" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="rejectDialogVisible = false">取消</el-button>
        <el-button type="danger" @click="handleReject" :loading="rejecting">確定拒絕</el-button>
      </template>
    </el-dialog>
  </div>
</template>
