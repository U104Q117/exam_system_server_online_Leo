<template>
  <div class="video-category-manage-container">
    <div class="action-bar">
      <el-button type="primary" @click="showAddDialog" icon="Plus">添加分類</el-button>
      <el-button @click="getCategoryList" icon="Refresh">刷新列表</el-button>
      <el-button @click="expandAll" icon="Expand">展開全部</el-button>
      <el-button @click="collapseAll" icon="Fold">摺疊全部</el-button>
    </div>
    ...
    <div class="category-list">
      <el-table :data="categoryList" v-loading="loading" stripe row-key="id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" :default-expand-all="false" ref="categoryTableRef">
        ...
        <el-table-column prop="videoCount" label="視頻數" width="100">
          <template #default="scope">
        ...
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="scope">
            <el-button size="small" @click="addChildCategory(scope.row)" icon="Plus" v-if="!scope.row.parentId || scope.row.level < 3">添加子分類</el-button>
            <el-button size="small" @click="editCategory(scope.row)" icon="Edit">編輯</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" :close-on-click-modal="false">
      <el-form :model="categoryForm" :rules="categoryRules" ref="categoryFormRef" label-width="100px">
        <el-form-item label="分類名稱" prop="name"><el-input v-model="categoryForm.name" placeholder="请输入分類名稱" /></el-form-item>
        <el-form-item label="分類描述" prop="description"><el-input v-model="categoryForm.description" type="textarea" :rows="3" placeholder="請輸入分類描述" /></el-form-item>
        <el-form-item label="父級分類" prop="parentId" v-if="!isAddChild">
          <el-tree-select v-model="categoryForm.parentId" :data="categoryTreeOptions" :props="{ label: 'name', value: 'id', children: 'children' }" placeholder="請選擇父級分類（不選擇則為頂級分類）" clearable check-strictly :render-after-expand="false" />
        </el-form-item>
        <el-form-item label="排序" prop="sortOrder"><el-input-number v-model="categoryForm.sortOrder" :min="0" :max="999" /></el-form-item>
        <el-form-item label="狀態" prop="status">
          <el-radio-group v-model="categoryForm.status"><el-radio :label="1">啟用</el-radio><el-radio :label="0">禁用</el-radio></el-radio-group>
        </el-form-item>
      </el-form>
      <template #footer>
