  ...
  <div class="app-container">
      ...
      <template v-if="activeView === 'questions'">
        ...
        <div class="search-bar">
          ...
          <el-row :gutter="20" class="search-row">
            ...
            <el-col :span="5">
              <el-select v-model="searchForm.type" placeholder="選擇類型" clearable @change="handleSearch">
                <el-option label="選擇題" value="CHOICE" />
                <el-option label="判断題" value="JUDGE" />
                <el-option label="簡答題" value="TEXT" />
              </el-select>
            </el-col>
            <el-col :span="5">
              <el-select v-model="searchForm.difficulty" placeholder="選擇難度" clearable @change="handleSearch">
          ...
          <el-row :gutter="20" class="action-row">
            <el-col :span="24">
              <div class="action-buttons">
                <el-button type="primary" @click="showAddDialog" icon="Plus">添加題目</el-button>
          ...
          <div class="question-section">
            <div class="question-list">
              <el-table :data="questionList" v-loading="loading" stripe @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="title" label="題目內容" min-width="300" show-overflow-tooltip />
                ...
                <el-table-column prop="difficulty" label="難度" width="100">
                  <template #default="{ row }">
                ...
                <el-table-column label="操作" width="180" fixed="right">
                  <template #default="{ row }">
                    <el-button size="small" @click="showEditDialog(row)" icon="Edit">編輯</el-button>
                    <el-button size="small" type="danger" @click="handleDelete(row)" icon="Delete">刪除</el-button>
                  </template>
                </el-table-column>
              </el-table>
    ...
    >
      <el-form :model="questionForm" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="題目類型" prop="type">
          <el-select v-model="questionForm.type" placeholder="請選擇題目類型" @change="onTypeChange">
            <el-option label="選擇題" value="CHOICE"></el-option>
            <el-option label="判断題" value="JUDGE"></el-option>
            <el-option label="簡答題" value="TEXT"></el-option>
          </el-select>
        </el-form-item>
        
        <el-form-item v-if="questionForm.type === 'CHOICE'" label="是否為多選題" prop="multi">
          <el-switch v-model="questionForm.multi"></el-switch>
        </el-form-item>

        <el-form-item label="分類" prop="categoryId">
          <el-select v-model="questionForm.categoryId" placeholder="選擇分類">
            <el-option 
        ...
        <el-form-item label="難度等級" prop="difficulty">
          <el-select v-model="questionForm.difficulty" placeholder="選擇難度等級">
            <el-option label="簡單" value="EASY" />
            <el-option label="中等" value="MEDIUM" />
            <el-option label="困難" value="HARD" />
          </el-select>
        ...
        <el-form-item label="題目解析" prop="analysis">
          <el-input
        ...
        <template v-if="questionForm.type === 'CHOICE'">
          <el-form-item
            ...
            :key="index"
            :label="'選項' + String.fromCharCode(65 + index)"
            :prop="'choices.' + index + '.content'"
            :rules="{ required: true, message: '請輸入選項內容', trigger: 'blur' }"
          >
            <div class="choice-item">
              <el-input v-model="choice.content" placeholder="請輸入選項內容" />
              <el-checkbox v-model="choice.isCorrect">正確答案</el-checkbox>  
              <el-button 
                type="danger" 
                icon="Delete" 
                circle 
        ...
        <template v-if="questionForm.type === 'JUDGE'">
          <el-form-item label="正確答案" prop="answer.answer">
    ...
    >
      <div class="batch-import-container">
        ...
        <el-steps :active="batchImportStep" finish-status="success" align-center>
          <el-step title="選擇文件" description="上傳Excel文件"></el-step>
          <el-step title="預覽數據" description="檢查導入數據"></el-step>
          <el-step title="導入完成" description="確認導入結果"></el-step> 
        </el-steps>
        ...
        <div class="step-content">
          ...
          <div v-if="batchImportStep === 0" class="upload-step">
            ...
            >
              <template #default>
                <ol>
                  <li>請先下載Excel模板，按照格式填寫題目數據</li>
                  <li>支持選擇題（單選/多選）、判断題、簡答題</li>
                  <li>Excel文件大小不超过10MB，題目數量建議不超过1000道</li>
                  <li>請確保必填欄位完整，數據格式正確</li>
                </ol>
              </template>
            </el-alert>
            ...
            >
              ...
              <div class="el-upload__text">
                將Excel文件拖曳到此，或<em>點擊上傳</em>
              </div>
              <template #tip>
                <div class="el-upload__tip">
                  只能上傳xlsx/xls文件，且不超过10MB
                </div>
              </template>
            </el-upload>
          ...
          <div v-if="batchImportStep === 1" class="preview-step">
            ...
            <div class="preview-table">
              <el-table :data="previewData" max-height="400" border>
                <el-table-column prop="title" label="題目內容" min-width="200" show-overflow-tooltip />
                ...
                <el-table-column prop="difficulty" label="難度" width="80">
                  <template #default="{ row }">
                ...
                <el-table-column prop="score" label="分數" width="60" />
                <el-table-column label="選項/答案" min-width="150">
                  <template #default="{ row }">
          ...
          <div v-if="batchImportStep === 2" class="result-step">
            ...
            >
              <template #extra>
                <el-button type="primary" @click="resetBatchImport">選擇導入</el-button>
                <el-button @click="batchImportDialogVisible = false">關閉</el-button>
              </template>
            </el-result>
          </div>
        </div>
    ...
    >
      <div class="ai-generate-container">
        ...
        <el-steps :active="aiGenerateStep" finish-status="success" align-center>
          <el-step title="設定參數" description="設定生成要求"></el-step>
          <el-step title="AI生成中" description="請稍候..."></el-step>
          <el-step title="预览确认" description="檢查生成結果"></el-step>
          <el-step title="导入完成" description="確認导入結果"></el-step>
        </el-steps>
        ...
        <div class="step-content">
          ...
          <div v-if="aiGenerateStep === 0" class="params-step">
            <el-form :model="aiGenerateForm" :rules="aiGenerateRules" ref="aiFormRef" label-width="120px">
              <el-form-item label="題目主題" prop="topic">
                <el-input
                  v-model="aiGenerateForm.topic"
                  placeholder="例如：Spring框架、Java併發編程、MySQL數據庫等"
                  clearable
                />
                <div class="form-tip">描述你想要生成题目的技術領域或知识点</div>
              </el-form-item>
              ...
              <el-form-item label="生成數量" prop="count">
                <el-input-number
                  ...
                  placeholder="1-20道題目"
                />
                <div class="form-tip">建議一次生成5-10道題目，品質更高</div>
              </el-form-item>

              <el-form-item label="題目類型" prop="types">
                <el-checkbox-group v-model="aiGenerateForm.types">
                  <el-checkbox label="CHOICE">選擇題</el-checkbox>
                  <el-checkbox label="JUDGE">判断題</el-checkbox>
                  <el-checkbox label="TEXT">簡答題</el-checkbox>
                </el-checkbox-group>
              </el-form-item>

              <el-form-item label="難度等級" prop="difficulty">
                <el-radio-group v-model="aiGenerateForm.difficulty">
                  <el-radio label="EASY">簡單</el-radio>
                  <el-radio label="MEDIUM">中等</el-radio>
                  <el-radio label="HARD">困難</el-radio>
                </el-radio-group>
              </el-form-item>

              <el-form-item label="類別分類" prop="categoryId">
                <el-select v-model="aiGenerateForm.categoryId" placeholder="選擇分類">
                  <el-option 
              ...
              <el-form-item label="包含多選題">
                <el-switch v-model="aiGenerateForm.includeMultiple" />
                <div class="form-tip">選擇題中是否包含多選題</div>
              </el-form-item>

              <el-form-item label="額外要求">
                <el-input
                  v-model="aiGenerateForm.requirements"
                  type="textarea"
                  :rows="3"
                  placeholder="例如：重點考察實際應用場景、包含代碼示例、侧重面试常考点等"
                />
                <div class="form-tip">可選，用於指引AI生成更符合需求的題目</div>
              </el-form-item>
            </el-form>

            <div class="step-actions">
              <el-button @click="aiGenerateDialogVisible = false">關閉</el-button>
          ...
          <div v-if="aiGenerateStep === 1" class="generating-step">
            <div class="generating-content">
              ...
              <h3>AI正在为您生成題目...</h3>
              <p>主題：{{ aiGenerateForm.topic }}</p>
              <p>數量：{{ aiGenerateForm.count }} 道</p>
              <p>類型：{{ getSelectedTypesText() }}</p>
          ...
          <div v-if="aiGenerateStep === 2" class="ai-preview-step">
            <div class="preview-header">
              <el-alert
                :title="`AI生成完成！共生成 ${aiGeneratedData.length} 道題目`"
                type="success"
                :closable="false"
                show-icon
                style="margin-bottom: 20px;"
            ...
            <div class="ai-preview-table">
              <el-table :data="aiGeneratedData" max-height="500" border>
                <el-table-column prop="title" label="題目內容" min-width="250" show-overflow-tooltip>
                ...
                <el-table-column label="選項/答案" min-width="200">
                  <template #default="{ row }">
                    <div v-if="row.type === 'CHOICE' && row.choices">
                      <div v-for="(choice, index) in row.choices" :key="index" class="choice-preview">
                        <span :class="{ 'correct-choice': choice.isCorrect }">
                ...
                <el-table-column label="操作" width="100" fixed="right">
                  <template #default="{ row, $index }">
                    <el-button size="small" type="primary" @click="editAiQuestion($index)" icon="Edit">編輯</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
          ...
          <div v-if="aiGenerateStep === 3" class="ai-result-step">
            ...
            >
              <template #extra>
                <el-button type="primary" @click="resetAiGenerate">繼續生成</el-button>
                <el-button @click="aiGenerateDialogVisible = false">關閉</el-button>
              </template>
            </el-result>
          </div>
        ...
        <template v-if="editingAiQuestion.type === 'CHOICE'">
          ...
          >
            <div class="choice-edit-item">
              ...
              <el-checkbox v-model="choice.isCorrect">正確</el-checkbox>
            </div>
          </el-form-item>
        </template>
