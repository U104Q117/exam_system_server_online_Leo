  ...
  <div class="category-manage-container">
    <div class="tips-section">
      <el-alert
        title="操作提示"
        type="info"
        description="管理考試題目分類，支持兩級分類。刪除分類前請確保該分類下沒有子分類和考試題目。"
        show-icon
        :closable="false">
      </el-alert>
    </div>
    ...
    <div class="tree-section">
      <el-table 
        ...
        <el-table-column prop="name" label="分類名稱" min-width="200"></el-table-column>
        <el-table-column label="級別" width="80">
        ...
        <el-table-column label="操作" width="320" fixed="right">
          <template #default="scope">
            ...
            <el-button 
              ...
              size="small" 
              type="success" 
              @click="addSubCategory(scope.row)">添加子分類</el-button>
            <el-button size="small" type="danger" @click="deleteCategory(scope.row)">刪除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="500px" @close="closeDialog">
      <el-form :model="categoryForm" :rules="formRules" ref="categoryFormRef" label-width="100px">
        <el-form-item label="分類名稱" prop="name">
          <el-input v-model="categoryForm.name" placeholder="請輸入分類名稱"></el-input>
        </el-form-item>
        <el-form-item v-if="isAddSubCategory" label="父分類" prop="parentId">
        ...
        <el-form-item label="排序序號" prop="sort">
          <el-input-number v-model="categoryForm.sort" :min="0" :max="999" placeholder="排序序號"></el-input-number>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog title="分类详情" v-model="viewDialogVisible" width="500px">
      <div class="category-detail" v-if="viewCategoryData">
        <div class="detail-row"><span class="label">分類名稱：</span><span class="value">{{ viewCategoryData.name }}</span></div>
        <div class="detail-row"><span class="label">分類級別：</span><span class="value"><el-tag :type="viewCategoryData.level === 1 ? 'primary' : 'success'">{{ viewCategoryData.level === 1 ? '一级分类' : '二级分类' }}</el-tag></span></div>
        ...
        <div class="detail-row"><span class="label">排序序號：</span><span class="value">{{ viewCategoryData.sort }}</span></div>
        <div class="detail-row"><span class="label">分類描述：</span><div class="value description">{{ viewCategoryData.description || '-' }}</div></div>
        <div class="detail-row"><span class="label">創建時間：</span><span class="value">{{ viewCategoryData.createTime }}</span></div>
        <div class="detail-row"><span class="label">更新时间：</span><span class="value">{{ viewCategoryData.updateTime }}</span></div>
      </div>
      <template #footer>
        <span class="dialog-footer"><el-button @click="viewDialogVisible = false">關閉</el-button></span>
      </template>
    </el-dialog>
  </div>
</template>
